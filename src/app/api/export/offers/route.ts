...
export async function GET(request: Request) {
  ...
  const rows = await prisma.offer.findMany({ where, orderBy: { [sort]: dir as any }, take: 1000 });
  const header = ["baslik", "tutar", "durum", "tarih"].join(",");
  const body = rows
    .map((r) => [r.title, r.amount, r.status, r.createdAt.toISOString()].map(csvEscape).join(","))
    .join("\n");
  const csv = header + "\n" + body + "\n";
