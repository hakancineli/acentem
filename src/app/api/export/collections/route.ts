...
export async function GET(request: Request) {
  ...
  const rows = await prisma.collection.findMany({ where, orderBy: { [sort]: dir as any }, take: 1000 });
  const header = ["tutar", "not", "tarih"].join(",");
  const body = rows
    .map((r) => [r.amount, r.note ?? "", r.createdAt.toISOString()].map(csvEscape).join(","))
    .join("\n");
  const csv = header + "\n" + body + "\n";
