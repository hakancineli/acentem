...
export default async function CollectionsPage({
  ...
  const { allowed } = await assertModuleEnabled("collections");
  if (!allowed) {
    return (
      ...
      <div className="rounded border p-4 space-y-3">
        <form className="flex items-center gap-2">
          ...
          <a
            ...
            href={`/api/export/collections?limit=${limit}&page=${page}&sort=${sort}&dir=${dir}${q ? `&q=${encodeURIComponent(q)}` : ""}${sp.start ? `&start=${encodeURIComponent(sp.start as string)}` : ""}${sp.end ? `&end=${encodeURIComponent(sp.end as string)}` : ""}`}
            target="_blank"
          >
            CSV Dışa Aktar
          </a>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="text-left text-neutral-500">
              ...
              <th>
                <Link href={`?sort=note&dir=${sort === "note" && dir === "asc" ? "desc" : "asc"}${q ? `&q=${encodeURIComponent(q)}` : ""}`}>Not</Link>
              </th>
              <th>
